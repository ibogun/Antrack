...
if __name__ == "__main__":
    ...
    for d in dataset.dictData:
        ...
        #for idx in range(minIdx, min(len(d["boxes"]), 2)):
        for idx in range(minIdx,min(len(d["boxes"]),len(d["images"]))):
            ...
            l_edgeness.append(objness.getEdgenessMultiscale(int(bbox[0]), int(bbox[1]), int(bbox[2]), int(bbox[3])))
            l_straddling.append(objness.getStraddlingMultiscale(int(bbox[0]), int(bbox[1]), int(bbox[2]), int(bbox[3])))
            ...
            for i in range(0, num_experiments - 1):
                ...
                for index in range(0, len(rho_values)):
                    ...
                    if experiment_box[0] >= 0 and experiment_box[1] >= 0 and experiment_box[0] + bbox[2] < m \
                        ...
                        edgeness = edgeness + objness.getEdgenessMultiscale(int(experiment_box[0]), int(experiment_box[1]),
                                                                  int(bbox[2]), int(bbox[3]))
                        straddling = straddling + objness.getStraddlingMultiscale(int(experiment_box[0]), int(experiment_box[1]),
                                                                        int(bbox[2]), int(bbox[3]))
    ...
    saveDictionaryAsCSV(dict_straddling,"straddling_multiscale.csv")
    saveDictionaryAsCSV(dict_edgeness,"edgeness_multiscale.csv")
