...
arma::mat Straddling::getLabels(cv::Mat &image) {
    ...
    arma::mat Label =
        seeds.calculateSegmentation(image, this->nSuperPixels, this->display);
    if (this->display) {
...
double Straddling::findStraddlingMeasure(arma::mat &labels, cv::Rect &rect) {
    ...
    arma::mat box = labels.submat(rect.x, rect.y, rect.x + rect.width - 1,
    ...
    for (int i = 0; i < uniqueLabels.size(); i++) {
        ...
        measure += MIN(pixelsForLabelInBox.size(),
                       (pixelsForLabel.size() - pixelsForLabelInBox.size())) /
    ...
    straddling = MAX(0, straddling);
    straddling = MIN(1, straddling);

    return straddling;
}

arma::rowvec Straddling::findStraddling(arma::mat &labels,
    ...
    for (int i = 0; i < rects.size(); i++) {
        ...
        cv::Rect translatedRect(rects[i].x - translate_x,
                                rects[i].y - translate_y, rects[i].width,
                                rects[i].height);
...
arma::rowvec Straddling::findStraddlng_fast(arma::mat &labels,
    ...
    for (int i = 0; i < rects.size(); i++) {
        ...
        cv::Rect rect_big(rects[i].x - translate_x, rects[i].y - translate_y,
...
void Straddling::straddlingOnCube(int img_rows, int img_cols, int center_x,
                                  int center_y, const std::vector<int> &R,
...
std::pair<std::vector<cv::Rect>, std::vector<double>>
Straddling::nonMaxSuppression(std::vector<arma::mat> &s, int n,
...
double Straddling::computeStraddling(cv::Rect &rect_big) {
    ...
    for (int superpixel = 0; superpixel < this->integrals.n_slices;
         superpixel++) {
        ...
        int area_superpixel_window_overlap =
            integrals(rect.x + rect.width, rect.y + rect.height, superpixel) +
        ...
        int area_superpixel_without_window =
            integrals(n, m, superpixel) - area_superpixel_window_overlap;
...
void EdgeDensity::edgeOnCube(int img_rows, int img_cols, int center_x,
                             int center_y, const std::vector<int> &R,
                             const std::vector<int> &w,
                             const std::vector<int> &h,
                             std::vector<arma::mat> &s) {
...
arma::rowvec EdgeDensity::findEdgeObjectness(std::vector<cv::Rect> &rects,
                                             int translate_x, int translate_y) {

    arma::rowvec measures(rects.size(), arma::fill::zeros);
    for (int i = 0; i < rects.size(); i++) {

        cv::Rect rect(rects[i].x - translate_x, rects[i].y - translate_y,
                      rects[i].width, rects[i].height);
...
double EdgeDensity::computeEdgeDensity(cv::Rect &rect) {
    ...
    int edges_in_x = edges_x(inner_rect.y, inner_rect.x + inner_rect.width) +
                     edges_x(inner_rect.y + inner_rect.height,
                             inner_rect.x + inner_rect.width) -
                     edges_x(inner_rect.y, inner_rect.x) -
                     edges_x(inner_rect.y + inner_rect.height, inner_rect.x);

    int edges_in_y = edges_y(inner_rect.y + inner_rect.height, inner_rect.x) +
                     edges_y(inner_rect.y + inner_rect.height,
                             inner_rect.x + inner_rect.width) -
                     edges_y(inner_rect.y, inner_rect.x) -
                     edges_y(inner_rect.y, inner_rect.x + inner_rect.width);

    //        int
    //        edges_in_x=edges_x(inner_rect.x+inner_rect.width,inner_rect.y)+edges_x(inner_rect.x+inner_rect.width,inner_rect.y+inner_rect.height)-edges_x(inner_rect.x,inner_rect.y)-edges_x(inner_rect.x,inner_rect.y+inner_rect.height);
    //
    //        int
    //        edges_in_y=edges_y(inner_rect.x,inner_rect.y+inner_rect.height)+edges_y(inner_rect.x+inner_rect.width,inner_rect.y+inner_rect.height)-edges_y(inner_rect.x,inner_rect.y)-edges_y(inner_rect.x+inner_rect.width,inner_rect.y);

    measure = (edges_in_x + edges_in_y) /
              ((double)(2 * (inner_rect.width + inner_rect.height)));
