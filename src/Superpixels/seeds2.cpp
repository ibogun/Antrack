...
cv::Mat SEEDS::getSegmentation(){
	
	cv::Mat segmentation(width,height,CV_8U);
	int i=0;
	
	for (int h=0;h<height;h++){
		for (int w=0;w<width;w++){
			segmentation.at<uchar>(w,h)=labels[seeds_top_level][i];
				i++;
		}
	}
...
void SEEDS::DrawContoursAroundSegments(UINT* img, UINT* labels, const int& width, const int& height, const UINT& color, bool internal)
{
