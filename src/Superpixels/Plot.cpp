...
void Plot::addPoint(double x, int lineNumber) {
    ...
    if (lines_dictionary.find(lineNumber) == lines_dictionary.end()) {
        ...
        int prevLocation_x =
            getCoordinateForX(lines_dictionary.at(lineNumber).getLast());
        ...
        if (debug) {
            std::cout << "Adding line from: " << prevPoint << " to "
...
int Plot::getYcoordinateForTime(int t) {
    ...
    if (t < 0 || t > this->numPts) {
        ...
        double delta_y = (this->canvas.cols - 2 * canvas.cols * origin_param) /
                         ((double)numPts);
