...
cv::Rect Struck::track(cv::Mat &image){
    ...
    if (useEdgeDensity || useStraddling) {
        ...
        if (useEdgeDensity) {
            arma::rowvec edge_objectness=weightWithEdgeDensity(smallImage,locationsOnaGrid,this->edge_params,min_x,min_y);

            predictions=predictions % edge_objectness;
        }

        if (useStraddling){
            arma::rowvec straddling_objectness=weightWithStraddling(smallImage,locationsOnaGrid,this->straddling_params,min_x,min_y);
            predictions=predictions % straddling_objectness;
        }
    ...
    if(this->scalePrior){
        ...
        for (int i=0; i<locationsOnaGrid.size(); i++) {
            scalePriors[i]=(1/(2*M_PI))*expf(-(sqrt(pow(locationsOnaGrid[i].width-
    ...
    //TODO: ADD DISPLAY==3 option here instead of other ...
 ...
 *
 *  @return vector of edge density measures, same size as the number of rectangles
 */
arma::rowvec Struck::weightWithEdgeDensity(cv::Mat &smallImage,
                                           std::vector<cv::Rect> &rects,std::vector<double> params,
                                           int min_x, int min_y){
    ...
    //EdgeDensity edgeDensity(edge_param1, edge_param2, inner_rectangle,objDisplay);
    //
...
//        arma::rowvec edge_measure_t=edgeDensity.findEdgeObjectness(edges, rects, min_x, min_y);
...
//    edge_measure=edge_measure/counter;
    ...
    if (objDisplay==3) {
        ...
        cv::Rect r3(smallImage.rows,imageCols+separation,smallImage.rows,smallImage.cols);

        this->copyFromRectangleToImage(this->objectnessCanvas, edges, r3, 2, cv::Vec3b(144,144,144));
    }
 ...
 */
arma::rowvec Struck::weightWithStraddling(cv::Mat &smallImage, std::vector<cv::Rect> &rects, std::vector<double> params, int min_x, int min_y){
...
//Straddling straddle(nSuperpixels,inner,objDisplay);
    ...
    arma::rowvec obj_measure_fast=this->straddle.findStraddlng_fast(labels, rects,min_x,min_y);
    ...
    if(objDisplay==3){
        ...
        this->copyFromRectangleToImage(this->objectnessCanvas, straddle.canvas, r2, 2, cv::Vec3b(144,144,144));
    ...
    for (int i=1; i<gt_images.second.size(); i++) {
        ...
        if (this->display==3) {
            ...
            if (this->useEdgeDensity || this->useStraddling) {
                ...
                this->copyFromRectangleToImage(plotImg, objPlot, bottomLeftRect, 0,cv::Vec3b(0,0,0));
...
Struck Struck::getTracker(){
    ...
    return getTracker(true, true,true,true, true);

}

/**
 *  Create Struck tracker object
 *
 *  @param pretraining    flag to use pretraining
 *  @param useFilter      flag to use robust kalman filter
 *  @param useEdgeDensity flag to use edge objectness
 *  @param useStraddling  flag to use straddling objectness
 *  @param scalePrior     flag to use scale priors
 *
 *  @return Struck object
 */
Struck Struck::getTracker(bool pretraining, bool useFilter,bool useEdgeDensity, bool useStraddling, bool scalePrior){
