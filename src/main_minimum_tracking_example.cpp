...
DEFINE_int32(display, 1, "Display settings (0 - no, 1 - basic, 2 - support "
                         "vectors, 3 - special (only for ObjStruck)).");
...
int main(int argc, char *argv[]) {
    ...
    if (tracker_type == 1) {
        tracker = new ObjDetectorStruck(pretraining, useFilter, useEdgeDensity,
        ...
        featureParamsMap.insert(
            std::make_pair("dis_features", FLAGS_dis_feature));
        ...
        tracker = new Struck(pretraining, useFilter, useEdgeDensity,
                             useStraddling, scalePrior, kernel, feature, note);
    }
    ...
    std::string prefix = "tracking_" + std::to_string(tracker_type) + "_" +
                         std::to_string(FLAGS_display) + "_";


    for (int i = 2; i < 10; i++) {
        std::string fileName = rootFolder + "000" + std::to_string(i) + ".jpg";
        tracker->track(fileName);
        std::cout << "Frame #" << i << " out of " << 10 << std::endl;
